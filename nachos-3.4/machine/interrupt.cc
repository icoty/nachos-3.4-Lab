...
//	interrupted thread.
//----------------------------------------------------------------------
// 从中断处理程序中调用，当处理程序返回时，在中断的线程中引起上下文切换(例如在时间片上)。
// 我们不能在这里进行上下文切换，因为这会关闭中断处理程序，我们想要关闭中断线程。
void
Interrupt::YieldOnReturn()
{ 
